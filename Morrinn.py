...
class Morrinn(Messages, GamePlay):
    ...
    map    = pygame.image.load('mapp.png')
    groke  = pygame.image.load('groke.png')
    heim = pygame.image.load('heim.png')
    ...
    def gameLoop(self):
        ...
        while not gameExit:
            ...
            if gameWin == True:
                self.stig = -1
                self.gameDisplay.blit(self.heim, [0,0, 800, 600])
                #self.gameDisplay.fill(self.white)   
            ...
            self.gameDisplay.blit(self.map, [0,0, 800, 600])
            #self.gameDisplay.fill(self.white)     
